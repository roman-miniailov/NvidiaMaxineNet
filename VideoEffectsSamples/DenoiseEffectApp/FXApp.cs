    ...
    internal class FXApp
    {
        ...
        Err initCamera(VideoCapture cap, string FLAG_camRes)
        {
            ...
            if (!string.IsNullOrEmpty(FLAG_camRes))
            {
                ...
                {
                    Debug.WriteLine("The requested resolution of %d x %d is not available and has been subsituted by %d x %d.\n", camWidth, camHeight, actualCamWidth, actualCamHeight);
                }
            }
        ...
        public Err processImage(string inFile, string outFile, float FLAG_strength)
        {
            ...
            if (!string.IsNullOrEmpty(outFile))
            {
                ...
                {
                    Debug.WriteLine("WARNING: JPEG output file format will reduce image quality\n");
                }
               
                if (!Cv2.ImWrite(outFile, _dstImg))
                {
                    Debug.WriteLine("Error writing: \"%s\"\n", outFile);
                    return Err.errWrite;
                }
            }
        ...
        public Err processMovie(Context context)
        {
            ...
            if (!reader.IsOpened())
            {
                if (!context.Webcam)
                {
                    Debug.WriteLine($"Error: Could not open video: {context.InFile}");
                }
                else
                {
                    Debug.WriteLine("Error: Webcam not found\n");
                }
            ...
            {
                Debug.WriteLine("Filters only target H264 videos, not %.4s\n", info.Codec);
            }
            ...
            if (!string.IsNullOrEmpty(context.OutFile))
            {
                ...
                if (!ok)
                {
                    Debug.WriteLine($"Cannot open {context.OutFile} for video writing.");
